...
class Inky:
    ...
    def setup(self):
        ...
        self._send_command(SSD2683_PSR, [0x2F, 0x69])
        self._send_command(SSD2683_BTST_P, [0x0F, 0x8B, 0x9C, 0x96])
        self._send_command(SSD2683_PWR, [0x07, 0xF0])
        self._send_command(SSD2683_CDI, [0x37])
        self._send_command(SSD2683_TRES, [X_ADDR_START_H, X_ADDR_START_L, Y_ADDR_START_H, Y_ADDR_START_L])
        self._send_command(0x62, [0x64, 0x53])
        self._send_command(SSD2683_GSST, [0x00, 0x00, 0x00, 0x00])
        self._send_command(SSD2683_PLL, [0x08])
    ...
    def _update(self, buf):
        ...
        self._send_command(SSD2683_PON)
        self._busy_wait()
        self._send_command(SSD2683_DTM, buf)
        self._send_command(SSD2683_DRF, [0x00])
        self._busy_wait()
        self._send_command(SSD2683_POF, [0x00])
        self._busy_wait()
        self._send_command(SSD2683_DSLP, [0xA5])
